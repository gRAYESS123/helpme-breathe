...
10. **Do not redesign.** Glassmorphism, Quicksand type, per-technique colour themes and the `.article-page` system in `css/styles.css` are assets. Extend them.
...
`<html lang="en">`, charset, viewport, `<title>` ≤ 65 chars, meta description 50–165 chars, absolute canonical, OG (title/description/type/url/image/site_name) + Twitter card, favicon (emoji SVG data URI as in index.html), apple-touch-icon, `manifest.json`, theme-color, preconnect + Quicksand font (non-blocking pattern from index.html), `css/styles.css`, GA4 snippet **with Consent Mode default denied** (copy exactly from `index.html` once Core has updated it), JSON-LD (`BreadcrumbList` on every page; `HowTo` + `FAQPage` on technique pages; `Article` on articles; `Product`/`Offer` only on `/pro`). Per-page OG image at `images/og/<slug>.jpg` (request generation via `integration_requests` if it does not exist).
