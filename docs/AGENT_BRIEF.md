...
- `js/ads.js` — `initAds()` injects the AdSense loader only when `!isPro()` and `document.body.dataset.noAds !== 'true'`, fills `.ad-slot[data-ad-slot]` elements, and never runs while `body.session-active`. `removeAds()` strips the containers outright for a paid tier, and `js/entitlements.js` stamps `document.body.dataset.tier` so CSS reserves no space either.
