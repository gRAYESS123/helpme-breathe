...
function selectTechnique(techniqueKey) {
    ...
    requestAnimationFrame(() => {
        ...
        if (circle) {
            // Reset to a clean breathing circle with only the current technique class
            circle.className = 'breathing-circle';
            circle.classList.add(technique.circleClass);

            // Force animation reset so the new technique starts cleanly
...
function startBreathing() {
    if (isPaused) {
        ...
        if (circle) {
            ...
            // Re-apply the current technique class and restart the animation
            const technique = techniques[currentTechnique];
    ...
    if (circle) {
        ...
        // Ensure the correct technique class is applied, then restart the animation
        const technique = techniques[currentTechnique];
        circle.classList.remove('technique-478', 'technique-box', 'technique-coherent', 'technique-triangle', 'technique-wim');
        circle.classList.add(technique.circleClass);
